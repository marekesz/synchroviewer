...
public abstract class AlgebraicModule {
  ...
  public static Rational[][] wordToMatrix(AbstractNFA automaton, String word) {
    ...
    for (int i = 0; i < n; i++) {
      ...
      for (int k = 0; k < word.length(); k++) {
        int char_id = charArrayIndexOf(Config.TRANSITIONS_LETTERS, word.charAt(k));
  ...
  private static Rational[] getFlatten(Rational[][] matrix) {
    Rational[] r = new Rational[matrix.length * matrix.length];
    for (int y = 0; y < matrix.length; y++)
      for (int x = 0; x < matrix.length; x++)
        r[y * matrix.length + x] = matrix[y][x];
    return r;
  }
