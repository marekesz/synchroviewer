...
public class ProbabilityDistributionToolbar extends DockToolbar implements ChangeListener {
  ...
  public ProbabilityDistributionToolbar(String name, boolean visibleOnStart, ProgramState programState) {
  ...
  protected void update() {
    int k = getProgramState().dfa.getK();
    if (letterSpinners.length != k) {
      ...
      for (int i = 0; i < k; i++) {
        c.gridwidth = 1;
        c.weightx = 0.0;
        letterLabels[i] = new JLabel(Character.toString(Config.TRANSITIONS_LETTERS[i]));
  ...
  private void setDistribution() {
    ...
    getProgramState().setProbabilityDistribution(dist);
  }
