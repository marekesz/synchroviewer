...
public class AlgebraicChainForSubsetToolbar extends DockToolbar {
  ...
  public AlgebraicChainForSubsetToolbar(String name, boolean visibleOnStart, ProgramState programState,
  ...
  private Pair<ArrayList<Integer>, String> getChainDescription(ArrayList<String> words, ArrayList<Rational[]> vectors,
    ...
    if (!foundExtendingWord) {
      if (changeSumSign > 0) text += "\nIncreasing-sum word not found";
      else if (changeSumSign < 0) text += "\nDecreasing-sum word not found";
    }
  ...
  private void recalculate() {
    ...
    AbstractNFA automaton = imageSelected ? getProgramState().dfa : new InverseDFA(getProgramState().dfa);

    Rational[] weights = null;

    if (eigenVectorPost || eigenVectorPre || eigenVectorZeroSum) {
      weights = getProgramState().getEigenVector();
      // Strong connectivity exception
      if (!Connectivity.isStronglyConnected(getProgramState().dfa, new InverseDFA(getProgramState().dfa))) {
    ...
    if (!increasingSum && !decreasingSum)
      results = LinAlgChain.linAlgChainForManySubsets(getProgramState().dfa, getProgramState().selectedStatesByColor, weights, zeroSum, eigenVectorPre,
        eigenVectorPost, eigenVectorZeroSum);
    else
      results = LinAlgChain.linAlgChainChangeSumForManySubsets(getProgramState().dfa, getProgramState().selectedStatesByColor, weights, zeroSum, eigenVectorPre,
        eigenVectorPost, eigenVectorZeroSum, increasingSum);

    int changeSumSign = 0;
    if (increasingSum) changeSumSign = 1;
    else if (decreasingSum) changeSumSign = -1;
